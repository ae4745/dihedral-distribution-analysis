...
All metrics are computed in `process_residue()` in `data_analysis.py`. To add a metric:

```python
from scipy.stats import entropy

kl_div = entropy(p, q)   # KL divergence (asymmetric; undefined if q has zero bins)
```

Add it to `output_lines` and `summary_rows` in the same function.
...
- Check that `DIHEDRAL_LABEL` in your config matches what is in the filenames
- Confirm extraction ran successfully (`output.txt` should have `[DONE]` lines)
- Run `ls simulation_*_chi1_*.csv` (or chi2) to verify files exist in the working directory

**`[SKIP] – missing files for one or both simulations`**
- A residue was found in one simulation but not the other (e.g. a residue absent in one structure)
- This is expected for mutant vs wild-type comparisons; these residues are skipped

**`[SKIP] – fewer than 4 atoms selected`**
- The atom selection string in `dihedral_definitions.py` does not match atom names in your topology
- Check with: `u.select_atoms("resname VAL and segid P0").atoms.names`

**`KeyError` or `NameError` on first run**
- Make sure you are running `dihedral_distribution_analysis.py`, not one of the sub-modules directly

**Slow extraction**
- Extraction uses `cpu_count() // 2` cores per repeat by default
- Adjust `n_workers` in `data_extraction.py` if needed
- For very large trajectories, consider pre-splitting them with MDAnalysis or GROMACS
